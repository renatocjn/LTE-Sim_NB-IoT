...
for r in $(seq $nRuns); do
	...
	seed=$((RANDOM * RANDOM))
 	(time bin/LTE-Sim_NBIot SingleCellM2mOnlyNbIot $cellRadius $nUe $ulScheduler $scClusterSize $seed) > $outDir/traceLteSim.txt 2> $outDir/time.txt &
done
done
wait
nChilds=0
