...
cellRadius="0.5" #500m
outFolder="quick_OnlyCBR_InfDelay"

#nUeList="25 50 75 100 125 150 175 200 225 250 275 300"
#nUeList="25 50 75"
nUeList="5 10 20 35 50 65"
...
for r in $(seq $nRuns); do
	...
	if [ ! -e $outDir/traceLteSim.txt ]; then 
		seed=$((RANDOM * RANDOM))
	 	(time bin/lte-sim-r5 SingleCellM2mUnderLTE $cellRadius $nUe cbr $ulScheduler  $seed) > $outDir/traceLteSim.txt 2> $outDir/time.txt &
	fi
done
done
wait
