...
ulSchedulerList="pf mt rr mlwdf expDelay"
...
for scClusterSize in $scClusterSizeList; do
	...
	seed=$((RANDOM * RANDOM))
 	(time bin/lte-sim-r5 SingleCellM2mUnderNbIotLongDelay $cellRadius $nUe $ulScheduler $scClusterSize $seed) > $outDir/traceLteSim.txt 2> $outDir/time.txt &
done
done
done
done
...
#for r in $(seq $nRuns); do
#
#if [ -e "Executions/$outFolder/clusterSizeComparation/ulscheduler=$ulScheduler/scClusterSize=$scClusterSize/nUe=$nUe/$r" ]; then
