...
void UplinkPacketScheduler::RBsAllocation() {
	...
	int nbOfRBs = users->at(0)->m_userToSchedule->GetProtocolStack()->GetMacEntity()->GetDevice()->GetPhy()->GetBandwidthManager()->GetUlSubChannels().size();
...
//			}
			...
			if (tbs <= dataToTransmit) {
				...
				int rbsNeeded = (floor)(scheduledUser->m_dataToTransmit / (GetMacEntity()->GetAmcModule()->GetTBSizeFromMCS(mcs, 1) / 8));
