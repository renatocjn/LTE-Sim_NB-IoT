...
void NbIotUlPfSchedulerV2::RBsAllocation() {
	...
	std::cout << "[NBIOT_DEBUG] nRU " << nRuToAllocate << std::endl;
	...
	for (int i = 0; i < nRuToAllocate; i++) {
		...
		sc += scGroupSize;
		std::cout << "[NBIOT_DEBUG] Selected user " << selectedUser->m_userToSchedule->GetIDNetworkNode() << " for RU " << i << " MCS: " << mcs
				<< " nRU: " << nRU << " txBytes: " << tbs << std::endl;
	}
