...
double UeMacEntity::GetSchedulingRequestInterval(void) {
	...
	if (rrc->GetRadioBearerContainer()->size() > 0) {
		...
		for (RrcEntity::RadioBearersContainer::iterator it = rrc->GetRadioBearerContainer()->begin(); it != rrc->GetRadioBearerContainer()->end(); it++) {
			...
			if (b->GetApplication()->GetApplicationType() != Application::APPLICATION_TYPE_INFINITE_BUFFER) {
				bufferStaturReport += b->GetMacQueue()->GetQueueSizeWithMACHoverhead();
//				std::cout << "SendSchedulingRequest UE " << this->GetDevice()->GetIDNetworkNode() << " byteSize " << bufferStaturReport << " pkgCount " << b->GetMacQueue()->GetNbDataPackets() << std::endl;
			} else {
...
void UeMacEntity::ScheduleUplinkTransmission(int nbPrb, int mcs) {
	int availableBytes = this->GetAmcModule()->GetTBSizeFromMCS(mcs, nbPrb) / 8;
...
void UeMacEntity::ChechFroDropPackets(void) {
	RrcEntity *rrc = GetDevice()->GetProtocolStack()->GetRrcEntity();
