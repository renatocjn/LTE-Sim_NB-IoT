...
void ChannelRealization::UpdateFastFading(void) {
	...
	if (GetSourceNode()->GetNodeType() == NetworkNode::TYPE_NBIOT_UE) {
		numbOfSubChannels = GetSourceNode()->GetPhy()->GetBandwidthManager()->GetUlSubChannels().size();
	} else if (GetDestinationNode()->GetNodeType() == NetworkNode::TYPE_NBIOT_UE) {
	...
	if (GetChannelType() == ChannelRealization::CHANNEL_TYPE_JAKES) {
		...
		if (GetChannelType() == ChannelRealization::CHANNEL_TYPE_PED_A) {
			for (int i = 0; i < numbOfSubChannels; i++) {
				...
				if (speed == 0) {
					for (int j = 0; j < samplingTime; j++) {
						ff_time.push_back(ff_PedA_speed_0[start_point_freq + i][start_point_time + j]);
					}
				}
				if (speed == 3) {
					for (int j = 0; j < samplingTime; j++) {
						ff_time.push_back(ff_PedA_speed_3[start_point_freq + i][start_point_time + j]);
					}
				}
				if (speed == 30) {
					for (int j = 0; j < samplingTime; j++) {
						ff_time.push_back(ff_PedA_speed_30[start_point_freq + i][start_point_time + j]);
					}
				}
				if (speed == 120) {
					for (int j = 0; j < samplingTime; j++) {
						ff_time.push_back(ff_PedA_speed_120[start_point_freq + i][start_point_time + j]);
					}
				}
				if (speed == 150) {
					for (int j = 0; j < samplingTime; j++) {
						ff_time.push_back(ff_PedA_speed_150[start_point_freq + i][start_point_time + j]);
					}
				}
				if (speed == 200) {
					for (int j = 0; j < samplingTime; j++) {
						ff_time.push_back(ff_PedA_speed_200[start_point_freq + i][start_point_time + j]);
					}
				}
				if (speed == 250) {
					for (int j = 0; j < samplingTime; j++) {
						ff_time.push_back(ff_PedA_speed_250[start_point_freq + i][start_point_time + j]);
					}
				}
				if (speed == 300) {
					for (int j = 0; j < samplingTime; j++) {
						ff_time.push_back(ff_PedA_speed_300[start_point_freq + i][start_point_time + j]);
					}
				}
				if (speed == 350) {
					for (int j = 0; j < samplingTime; j++) {
						ff_time.push_back(ff_PedA_speed_350[start_point_freq + i][start_point_time + j]);
					}
				}
				m_fastFading->push_back(ff_time);
			}
		...
		if (GetChannelType() == ChannelRealization::CHANNEL_TYPE_PED_B) {
			for (int i = 0; i < numbOfSubChannels; i++) {
				...
				if (speed == 0) {
					for (int j = 0; j < samplingTime; j++) {
						ff_time.push_back(ff_PedB_speed_0[start_point_freq + i][start_point_time + j]);
					}
				}
				if (speed == 3) {
					for (int j = 0; j < samplingTime; j++) {
						ff_time.push_back(ff_PedB_speed_3[start_point_freq + i][start_point_time + j]);
					}
				}
				if (speed == 30) {
					for (int j = 0; j < samplingTime; j++) {
						ff_time.push_back(ff_PedB_speed_30[start_point_freq + i][start_point_time + j]);
					}
				}
				if (speed == 120) {
					for (int j = 0; j < samplingTime; j++) {
						ff_time.push_back(ff_PedB_speed_120[start_point_freq + i][start_point_time + j]);
					}
				}
				m_fastFading->push_back(ff_time);
			}
		...
		if (GetChannelType() == ChannelRealization::CHANNEL_TYPE_VEH_A) {
			for (int i = 0; i < numbOfSubChannels; i++) {
				...
				if (speed == 0) {
					for (int j = 0; j < samplingTime; j++) {
						ff_time.push_back(ff_VehA_speed_0[start_point_freq + i][start_point_time + j]);
					}
				}
				if (speed == 3) {
					for (int j = 0; j < samplingTime; j++) {
						ff_time.push_back(ff_VehA_speed_3[start_point_freq + i][start_point_time + j]);
					}
				}
				if (speed == 30) {
					for (int j = 0; j < samplingTime; j++) {
						ff_time.push_back(ff_VehA_speed_30[start_point_freq + i][start_point_time + j]);
					}
				}
				if (speed == 120) {
					for (int j = 0; j < samplingTime; j++) {
						ff_time.push_back(ff_VehA_speed_120[start_point_freq + i][start_point_time + j]);
					}
				}
				if (speed == 150) {
					for (int j = 0; j < samplingTime; j++) {
						ff_time.push_back(ff_VehA_speed_150[start_point_freq + i][start_point_time + j]);
					}
				}
				if (speed == 200) {
					for (int j = 0; j < samplingTime; j++) {
						ff_time.push_back(ff_VehA_speed_200[start_point_freq + i][start_point_time + j]);
					}
				}
				if (speed == 250) {
					for (int j = 0; j < samplingTime; j++) {
						ff_time.push_back(ff_VehA_speed_250[start_point_freq + i][start_point_time + j]);
					}
				}
				if (speed == 300) {
					for (int j = 0; j < samplingTime; j++) {
						ff_time.push_back(ff_VehA_speed_300[start_point_freq + i][start_point_time + j]);
					}
				}
				if (speed == 350) {
					for (int j = 0; j < samplingTime; j++) {
						ff_time.push_back(ff_VehA_speed_350[start_point_freq + i][start_point_time + j]);
					}
				}
				m_fastFading->push_back(ff_time);
			}
		...
		if (GetChannelType() == ChannelRealization::CHANNEL_TYPE_VEH_B) {
			for (int i = 0; i < numbOfSubChannels; i++) {
				...
				if (speed == 0) {
					for (int j = 0; j < samplingTime; j++) {
						ff_time.push_back(ff_VehB_speed_0[start_point_freq + i][start_point_time + j]);
					}
				}
				if (speed == 3) {
					for (int j = 0; j < samplingTime; j++) {
						ff_time.push_back(ff_VehB_speed_3[start_point_freq + i][start_point_time + j]);
					}
				}
				if (speed == 30) {
					for (int j = 0; j < samplingTime; j++) {
						ff_time.push_back(ff_VehB_speed_30[start_point_freq + i][start_point_time + j]);
					}
				}
				if (speed == 120) {
					for (int j = 0; j < samplingTime; j++) {
						ff_time.push_back(ff_VehB_speed_120[start_point_freq + i][start_point_time + j]);
					}
				}
				if (speed == 150) {
					for (int j = 0; j < samplingTime; j++) {
						ff_time.push_back(ff_VehB_speed_150[start_point_freq + i][start_point_time + j]);
					}
				}
				if (speed == 200) {
					for (int j = 0; j < samplingTime; j++) {
						ff_time.push_back(ff_VehB_speed_200[start_point_freq + i][start_point_time + j]);
					}
				}
				if (speed == 250) {
					for (int j = 0; j < samplingTime; j++) {
						ff_time.push_back(ff_VehB_speed_250[start_point_freq + i][start_point_time + j]);
					}
				}
				if (speed == 300) {
					for (int j = 0; j < samplingTime; j++) {
						ff_time.push_back(ff_VehB_speed_300[start_point_freq + i][start_point_time + j]);
					}
				}
				if (speed == 350) {
					for (int j = 0; j < samplingTime; j++) {
						ff_time.push_back(ff_VehB_speed_350[start_point_freq + i][start_point_time + j]);
					}
				}
				m_fastFading->push_back(ff_time);
			}
