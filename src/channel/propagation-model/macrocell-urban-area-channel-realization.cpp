...
double MacroCellUrbanAreaChannelRealization::GetPathLoss(void) {
	...
	distance = src->GetMobilityModel()->GetAbsolutePosition()->GetDistance(dst->GetMobilityModel()->GetAbsolutePosition());
...
std::vector<double> MacroCellUrbanAreaChannelRealization::GetLoss() {
	...
	if (GetSourceNode()->GetNodeType() == NetworkNode::TYPE_ENODEB) {
		...
		if (GetDestinationNode()->GetNodeType() == NetworkNode::TYPE_NBIOT_UE) {
			nbOfSubChannels = GetDestinationNode()->GetPhy()->GetBandwidthManager()->GetDlSubChannels().size();
		}
	} else {
	...
	for (int i = 0; i < nbOfSubChannels; i++) {
		FastFading *ff = GetFastFading();
		FastFadingForTimeDomain v = ff->at(i);

		double l = ff->at(i).at(index) - GetPathLoss() - GetPenetrationLoss() - GetShadowing();
		loss.push_back(l);
