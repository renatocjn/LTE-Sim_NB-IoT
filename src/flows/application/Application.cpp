...
void Application::Start() {
	...
	if (GetSource()->GetNodeType() == NetworkNode::TYPE_UE || GetSource()->GetNodeType() == NetworkNode::TYPE_NBIOT_UE) {
...
#ifdef TEST_START_APPLICATION
		std::cout << "Create UL radio bearer between: " << GetSource ()->GetIDNetworkNode ()
...
TransportProtocol::TransportProtocolType Application::GetTransportProtocol(void) const {
...
void Application::Trace(Packet* p) {
	...
	if (GetDestination()->GetNodeType() == NetworkNode::TYPE_UE || GetDestination()->GetNodeType() == NetworkNode::TYPE_NBIOT_UE) {
		...
		std::cout << " ID " << p->GetID() << " B " << GetApplicationID() << " SIZE " << p->GetSize() << " SRC " << GetSource()->GetIDNetworkNode()
				<< " DST " << GetDestination()->GetIDNetworkNode() << " T " << Simulator::Init()->Now() << std::endl;
	}
}
