...
RadioBearer::CreatePacket(int bytes) {
	...
	if (_APP_TRACING_) {
		...
		std::cout << " ID " << p->GetID() << " B " << GetRlcEntity()->GetRlcEntityIndex() << " SIZE " << bytes << " SRC " << GetSource()->GetIDNetworkNode() << " DST " << GetDestination()->GetIDNetworkNode() << " T " << Simulator::Init()->Now() << " " << ue->IsIndoor() << std::endl;
	}
...
//			std::cout << "Queue size " << GetMacQueue()->GetNbDataPackets() << " Queue byte size " << GetMacQueue()->GetQueueSizeWithMACHoverhead() << std::endl;
